...
const { data: profiles, pending } = await useAsyncData(`seo-skill-base-${skillSlug}`, async () => {
  return await $fetch<any[]>(`${config.public.apiUrl}/profiles`, {
    params: { skill: skillData.shortName }
  });
});

const title = `Заказать ${skillData.nameInCase} — База специалистов и фрилансеров`;
const description = `Ищете специалиста? Каталог проверенных исполнителей, чтобы заказать ${skillData.nameInCase}. Прямые контакты без посредников и комиссий на fyxi.ru.`;
    ...
    <div class="seo-container">
      ...
      <nav class="breadcrumbs" aria-label="Breadcrumb">
        <ol>
          ...
          <li><span class="separator">/</span></li>
          <li aria-current="page">{{ skillData.name }}</li>
        </ol>
      </nav>
      ...
      <header class="seo-hero">
        <div class="hero-content">
          ...
          <h1 class="page-title">Заказать <span class="gradient-text">{{ skillData.nameInCase }}</span></h1>
          <p class="page-desc">{{ skillData.description }} Мы собрали лучших фрилансеров и студии, готовых взяться за ваш проект без комиссий посредников. Работайте напрямую.</p>
        </div>
      </header>
      ...
      <section class="seo-text-block">
        ...
        <div class="seo-content-grid">
          ...
          <div class="seo-card">
            ...
            <p>Наша база включает профессионалов со всего СНГ, готовых работать удаленно. Это позволяет найти лучшее соотношение цены и качества.</p>
          </div>
        </div>
      </section>
